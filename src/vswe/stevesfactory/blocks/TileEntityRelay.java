...
public class TileEntityRelay extends TileEntityClusterElement implements IInventory, ISidedInventory, IFluidHandler, ITileEntityInterface {
    ...
    public void setOwner(EntityLivingBase entity) {
        if (entity != null && entity instanceof EntityPlayer) {
            owner = StringUtils.stripControlCodes(((EntityPlayer)entity).username);
        }
    }
    ...
    private boolean isAdvanced() {
        return Blocks.blockCableRelay.isAdvanced(getBlockMetadata());
    }
    ...
    private <T> T getContainer(Class<T> type, int id) {
        ...
        ForgeDirection direction = ForgeDirection.VALID_DIRECTIONS[Blocks.blockCableRelay.getSideMeta(getBlockMetadata()) % ForgeDirection.VALID_DIRECTIONS.length];
    ...
    private boolean isPlayerActive(EntityPlayer player) {
        if (player != null) {
            for (UserPermission permission : permissions) {
                if (permission.getName().equals(StringUtils.stripControlCodes(player.username))) {
    ...
    public void readContentFromNBT(NBTTagCompound nbtTagCompound) {
        ...
        if (nbtTagCompound.hasKey(NBT_OWNER)) {
            ...
            NBTTagList permissionTags = nbtTagCompound.getTagList(NBT_PERMISSIONS);
            for (int i = 0; i < permissionTags.tagCount(); i++) {
                NBTTagCompound permissionTag = (NBTTagCompound)permissionTags.tagAt(i);
